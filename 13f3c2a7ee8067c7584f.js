...
const toggleGameTurn = () => {
  ...
  if (gameTurn == "player1") p2gameboardGrid.style.pointerEvents = "auto";else p2gameboardGrid.style.pointerEvents = "none";
  // if (gameTurn == "player1") {
...
const renderGameboard = (gameboardGrid, gameboardcells) => {
  ...
  for (let i = 0; i < gameboardcells.length; i++) for (let j = 0; j < gameboardcells[i].length; j++) {
    const div = document.createElement("div");
    if (gameboardcells[i][j] != null && gameboardcells[i][j] != "X") div.classList.add("shipCell");
